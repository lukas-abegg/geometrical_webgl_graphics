...
function paint_hyperbolic_helicoid() {
// Get the WebGL context.
    var canvas = document.getElementById('canvas_hyperbolic_helicoid');
...
//    gl.vertexAttrib4f(colAttrib, 0, 0, 1, 1);
    ...
    function createVertexData() {
        ...
        // Loop angle t.
        for (var i = 0, u = -4; i <= n; i++, u += du) {
            // Loop radius r.
            for (var j = 0, v = -4; j <= m; j++, v += dv) {
                ...
                var y = Math.sinh(v) * Math.sin(a * u) / (1 + Math.cosh(u) * Math.cosh(v)) / 2;
                ...
                if(v > -2 && v <= 0){
                    colors[iVertex * 4] = 0;
                    colors[iVertex * 4 + 1] = 1-v * (-1) * 0.25;
                    colors[iVertex * 4 + 2] = v *(-1) * 0.25;
                    colors[iVertex * 4 + 3] = 1;
                }
                if(v > 0 && v <= 2){
                    colors[iVertex * 4] = 0;
                    colors[iVertex * 4 + 1] = v * 0.25;
                    colors[iVertex * 4 + 2] = v * 0.25;
                    colors[iVertex * 4 + 3] = 1;
                }
                if(v > 2){
                    colors[iVertex * 4] = v * 0.25;
                    colors[iVertex * 4 + 1] = v * 0.25;
                    colors[iVertex * 4 + 2] = 0;
                    colors[iVertex * 4 + 3] = 1;
                }
...
window.addEventListener("load", paint_hyperbolic_helicoid, true);